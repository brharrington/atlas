...
/**
  * URI analysis that wraps an [[AtlasDocumentAnalyzer]] via composition. Parses
...
class UriDocumentAnalyzer(
  private val asl: AtlasDocumentAnalyzer,
  ...
  /**
    * Translate a cursor offset in the raw URI value to the corresponding
    * offset in the decoded expression string. For non-encoded values this
    * is a simple subtraction; for percent-encoded values we use the offset
    * map to handle the size difference.
    */
  private def rawOffsetToDecoded(param: QueryParam, uriOffset: Int): Int = {
    ...
    val map = UriParser.buildOffsetMap(param.value.text)
    // Binary search or linear scan for the decoded index whose raw offset
    // is <= rawRelative. Linear scan is fine for typical URI lengths.
    var di = 0
  ...
  override def computeSemanticTokens(text: String): List[Integer] = {
    ...
    if (parsed.path.end > parsed.path.start) {
      tokens += ((parsed.path.start, parsed.path.end - parsed.path.start, AtlasTokenTypes.Path))
    }

    // ? operator
    val qIdx = text.indexOf('?')
    if (qIdx >= 0) {
      tokens += ((qIdx, 1, AtlasTokenTypes.UriOperator))
    }
    ...
    for (param <- parsed.query) {
      ...
      if (param.key.start > 0 && param.key.start > parsed.path.end + 1) {
        ...
        if (ampPos >= 0 && ampPos < text.length && text.charAt(ampPos) == '&') {
          tokens += ((ampPos, 1, AtlasTokenTypes.UriOperator))
        }
      }
      ...
      tokens += ((
        ...
        AtlasTokenTypes.Parameter
      ))

      // = operator (only if there's a value span)
      if (param.key.end < param.value.start) {
        tokens += ((param.key.end, 1, AtlasTokenTypes.UriOperator))
      }
  ...
  override def computeCodeActions(uri: String): List[Either[Command, CodeAction]] = {
    ...
    parsed.query
      .filter(isExpression)
      .flatMap { param =>
        val tempUri = s"$uri#${param.key.text}"
  ...
  private[lsp] def publishDiagnostics(uri: String, text: String): Unit = {
    ...
    for (param <- parsed.query if isExpression(param)) {
      ...
      val capturingClient = new CapturingLanguageClient(captured)
      val tempAnalyzer = new AtlasDocumentAnalyzer(
        asl.interpreter,
        asl.glossary,
        () => capturingClient
      )
      tempAnalyzer.updateDocument(tempUri, param.decodedValue)
      val aslDiags = captured.get()
      if (aslDiags != null) {
        for (d <- aslDiags.getDiagnostics.asScala) {
      ...
      tempAnalyzer.removeDocument(tempUri)
    }
